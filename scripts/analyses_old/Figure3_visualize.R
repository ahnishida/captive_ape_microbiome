...
ASV_table <- read.table("results/gyrb/analyses/figures/Figure3_table.txt",sep='\t',header=TRUE)
...
ggsave('results/gyrb/analyses/figures/Bt2_tree.pdf',width=4)
...
ggtree(Bt3_tree_cls, aes(color=group)) + xlim(NA, .8) +
 ...
 # geom_nodepoint(aes(subset = label > .70),size=.75) + 
  geom_treescale(x=0,y=0) +
#bootstrap
  geom_cladelabel(node=625,label='Bt3_clade1_human',color='dodgerblue3',offset=.30) +
  geom_cladelabel(node=1057,label='Bt3_clade1_chimp',color='darkorange2',offset=.25) +
  geom_cladelabel(node=1077,label='Bt3_clade1_bonobo',color='red2',offset=.25) +
...
ggsave('results/gyrb/analyses/figures/Bt3_tree.pdf',width=7)
...
inset_table_Bt2 <- inset_table %>% dplyr::filter(ASV %in% Bt2_inset_tree$tip.label)
inset_table_Bt3 <- inset_table %>% dplyr::filter(ASV %in% Bt3_inset_tree$tip.label)

Bt2_inset_tree <- keep.tip(Bt2_inset_tree,inset_table_Bt2$ASV)
Bt3_inset_tree <- keep.tip(Bt3_inset_tree,inset_table_Bt3$ASV)

add_HRclades_to_tree_codiv <- function(lineage_tree,table){
  (taxa<-lineage_tree$tip.label) 
  (lineage_table <- table %>% 
      dplyr::filter(ASV %in% taxa & cladeName != "") %>% 
      dplyr::group_by(cladeName) %>% 
...
Bt2_inset_tree_cls <- add_HRclades_to_tree_codiv(Bt2_inset_tree,inset_table)
...
ggtree(Bt2_inset_tree_cls, aes(color=group)) + xlim(NA, .2) +
  ...
  geom_treescale()
ggsave('results/gyrb/analyses/figures/Bt2_inset_tree.pdf')


Bt3_inset_tree_cls <- add_HRclades_to_tree_codiv(Bt3_inset_tree,inset_table)
...
ggtree(Bt3_inset_tree_cls, aes(color=group)) + xlim(NA, .4) +
  ...
  geom_treescale()
ggsave('results/gyrb/analyses/figures/Bt3_inset_tree.pdf')
