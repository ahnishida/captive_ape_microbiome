...
dir.create('results/16s/analyses/tables/')
...
write.table(jaccard_df,file='results/16s/analyses/tables/pw_betadisper_permanova_jaccard.txt',sep='\t',row.names = F,col.names=F)
...
write.table(bray_df,file='results/16s/analyses/tables/pw_betadisper_permanova_braycurtis.txt',sep='\t',row.names = F,col.names=F)
...
write.table(unifrac_df,file='results/16s/analyses/tables/pw_betadisper_permanova_unweighted_unifrac.txt',sep='\t',row.names = F,col.names=F)
...
write.table(wunifrac_df,file='results/16s/analyses/tables/pw_betadisper_permanova_weighted_unifrac.txt',sep='\t',row.names = F,col.names=F)
...
df_sig <- df %>% filter(non_industrialized_humanvs.captive<.05|industrialized_humanvs.captive<.05|industrialized_humanvs.non_industrialized_human<.05|
                          wildvs.captive<.05|wildvs.non_industrialized_human<.05|wildvs.industrialized_human<.05) 

#genera that differ significantly in at least one pairwise comparison  and reach 1% abundance in at least one group
df_sig_over01 <- df_sig %>% filter(wild_mean>.01|captive_mean>.01|nih_mean>.01|ih_mean>.01)
...
ggsave('analyses/figures/Fig_relative_abundance_common_genera_16S.pdf',width=9)
