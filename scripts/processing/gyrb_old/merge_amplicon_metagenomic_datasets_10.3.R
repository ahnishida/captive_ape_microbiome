...
rootTree(file.path(outpath,folder,"ASVs_filtered_ref_aln_trim.tre"),
         file.path(outpath,folder,"ASVs_filtered_ref_aln_trim_rooted.tre"),
         outgroup = c('o__AKYH767','o__NS11-12g'))
############
...
ref_seqs/Bt_out_aln.fasta
gyrb_ref_fasta = 'results/gyrb/processing/ref_gyrb_gtdbtk/gyrb_fastas/gtdbtk_gyrb_Bt.fasta'
...
table(order[class == "c__Bacteroidia"]) 
table(genus[order == "o__Flavobacteriales"]) 

Bacteroidia_ref <- gyrb_ref[order %in% c('o__Bacteroidales')]
Bacteroidia_ref <- gyrb_ref[order %in% c('o__Bacteroidales')]
...
asv_ref <- c(gyrb_ref,DNAStringSet(asv_fasta_filt,start=2))
asv_ref_aln <- AlignTranslation(asv_ref)
outpath='results/gyrb/processing/gyrb_amp_meta_datasets_10.2/results_delta'
dir.create(outpath, recursive=TRUE)
Biostrings::writeXStringSet(asv_ref_aln,file=file.path(outpath,"ASVs_filtered_aln.fasta")) 
asv_ref_aln_trim <- DNAStringSet(asv_ref_aln,start=349)
Biostrings::writeXStringSet(asv_ref_aln_trim,file=file.path(outpath,"ASVs_filtered_aln_trim.fasta")) 
#run fasttree
system(paste0('./FastTree-2.1.9 -nt -gtr <  ',
              file.path(outpath,"ASVs_filtered_aln.fasta"),
              ' > ',
              file.path(outpath,"ASVs_filtered_aln.tre")))
