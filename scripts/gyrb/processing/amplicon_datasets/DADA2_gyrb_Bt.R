...
source('scripts/gyrb/processing/amplicon_datasets/DADA2_single.R')
...
#now select only Bt samples
(Bt_forward_reads <- forward_reads[stringr::str_detect(sample.names,'Bt')])
(Bt_sample.names <- sample.names[stringr::str_detect(sample.names,'Bt')])
